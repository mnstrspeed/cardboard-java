...
public class HeadMountedDisplayManager {
	...
	public HeadMountedDisplayManager(final Context context) {
		...
		this.mHmd = new HeadMountedDisplay(this.createScreenParams(),
	...
	public void onResume() {
		final CardboardDeviceParams deviceParams = this
				.createCardboardDeviceParamsFromExternalStorage();
		if (deviceParams != null
				&& !deviceParams.equals(this.mHmd.getCardboardDeviceParams())) {
		...
		final ScreenParams screenParams = this
				.createScreenParamsFromExternalStorage(this.getDisplay());
		if (screenParams != null
				&& !screenParams.equals(this.mHmd.getScreenParams())) {
	...
	public boolean updateCardboardDeviceParams(
		...
		if (cardboardDeviceParams == null
				|| cardboardDeviceParams.equals(this.mHmd
	...
	private void writeCardboardParamsToExternalStorage() {
		...
		try {
			...
			success = this.mHmd.getCardboardDeviceParams().writeToOutputStream(
					stream);
	...
	private ScreenParams createScreenParams() {
		...
		final ScreenParams params = this
				.createScreenParamsFromExternalStorage(display);
		if (params != null) {
			Log.i("HeadMountedDisplayManager",
	...
	private CardboardDeviceParams createCardboardDeviceParams() {
		CardboardDeviceParams params = this
				.createCardboardDeviceParamsFromExternalStorage();
		if (params != null) {
			Log.i("HeadMountedDisplayManager",
	...
	private CardboardDeviceParams createCardboardDeviceParamsFromAssetFolder() {
		try {
			...
			Log.d(s,
					new StringBuilder(47 + value.length())
							.append("Bundled Cardboard device parameters not found: ")
							.append(value).toString());
		} catch (IOException e2) {
			final String s2 = "HeadMountedDisplayManager";
			final String value2 = String.valueOf(String.valueOf(e2));
			Log.e(s2,
					new StringBuilder(43 + value2.length())
							.append("Error reading config file in asset folder: ")
	...
	private CardboardDeviceParams createCardboardDeviceParamsFromExternalStorage() {
		try {
			...
			final String value = String.valueOf(String.valueOf(e));
			Log.d(s,
					new StringBuilder(44 + value.length())
							.append("Cardboard device parameters file not found: ")
							.append(value).toString());
			return null;
		}
	}
	...
	private ScreenParams createScreenParamsFromExternalStorage(
		...
		try {
			...
			Log.d(s,
					new StringBuilder(44 + value.length())
							.append("Cardboard screen parameters file not found: ")
							.append(value).toString());
			return null;
		}
	}
