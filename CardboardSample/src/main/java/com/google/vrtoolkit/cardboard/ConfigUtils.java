...
public abstract class ConfigUtils {
	...
	public static File getConfigFile(final String filename) {
		final File configFolder = new File(
				Environment.getExternalStorageDirectory(), "Cardboard");
		if (!configFolder.exists()) {
			configFolder.mkdirs();
		} else if (!configFolder.isDirectory()) {
			final String value = String.valueOf(String.valueOf(configFolder));
			throw new IllegalStateException(new StringBuilder(
					22 + value.length()).append("Folder ").append(value)
					.append(" already exists").toString());
		}
		return new File(configFolder, filename);
	}

	public static InputStream openAssetConfigFile(
			final AssetManager assetManager, final String filename)
			throws IOException {
