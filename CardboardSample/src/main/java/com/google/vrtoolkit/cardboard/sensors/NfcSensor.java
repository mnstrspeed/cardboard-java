...
public class NfcSensor {
	...
	private NfcSensor(final Context context) {
		...
		final IntentFilter ndefIntentFilter = new IntentFilter(
	...
	public void removeOnCardboardNfcListener(
			final OnCardboardNfcListener listener) {
		if (listener == null) {
			return;
		}
		synchronized (this.mListeners) {
	...
	public NdefMessage getTagContents() {
		synchronized (this.mTagLock) {
			return (this.mCurrentNdef != null) ? this.mCurrentNdef
	...
	public void writeUri(final Uri uri) throws TagLostException, IOException,
		...
		synchronized (this.mTagLock) {
			...
			if (currentMessage != null) {
				...
				newMessage = new NdefMessage(
						(NdefRecord[]) newRecords
								.toArray(new NdefRecord[newRecords.size()]));
			}
			if (newMessage == null) {
			...
			Label_0432: {
				if (this.mCurrentNdef != null) {
					...
					if (this.mCurrentNdef.getMaxSize() < newMessage
							.getByteArrayLength()) {
						throw new IllegalArgumentException(
								new StringBuilder(82)
										.append("Not enough capacity in NFC tag. Capacity: ")
										.append(this.mCurrentNdef.getMaxSize())
										.append(" bytes, ")
										.append(newMessage.getByteArrayLength())
					...
					try {
						...
						throw new RuntimeException(
								(value.length() != 0) ? s.concat(value)
										: new String(s));
					}
				}
				final NdefFormatable ndef = NdefFormatable
						.get(this.mCurrentTag);
				if (ndef == null) {
					throw new IOException(
							"Could not find a writable technology for the NFC tag");
				}
				Log.w("NfcSensor",
				...
				try {
					...
					throw new RuntimeException(
							(value2.length() != 0) ? s2.concat(value2)
									: new String(s2));
				}
			}
	...
	public void onNfcIntent(final Intent intent) {
		if (!this.isNfcEnabled() || intent == null
				|| !this.mNfcIntentFilters[0].matchAction(intent.getAction())) {
			return;
		}
		this.onNewNfcTag((Tag) intent
	...
	private void onNewNfcTag(final Tag nfcTag) {
		...
		synchronized (this.mTagLock) {
			...
			if (previousNdef != null) {
				...
				isSameTag = (tagId1 != null && tagId2 != null && Arrays.equals(
						tagId1, tagId2));
			...
			this.mCurrentTagIsCardboard = this
					.isCardboardNdefMessage(nfcTagContents);
						...
						.schedule(new TimerTask() {
							...
							public void run() {
								synchronized (NfcSensor.this.mTagLock) {
									if (!NfcSensor.this.mCurrentNdef
											.isConnected()) {
	...
	private static class ListenerHelper implements OnCardboardNfcListener {
		...
		public ListenerHelper(final OnCardboardNfcListener listener,
