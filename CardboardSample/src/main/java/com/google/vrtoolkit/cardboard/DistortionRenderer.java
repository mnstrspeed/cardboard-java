...
public class DistortionRenderer {
	...
	public void setTextureFormat(final int textureFormat, final int textureType) {
		if (this.mDrawingFrame) {
			throw new IllegalStateException(
					"Cannot change texture format during rendering.");
		}
		if (textureFormat != this.mTextureFormat
				|| textureType != this.mTextureType) {
	...
	public void undistortTexture(final int textureId) {
		...
		GLES20.glViewport(0, 0, this.mHmd.getScreenParams().getWidth(),
		...
		GLES20.glScissor(0, 0, this.mHmd.getScreenParams().getWidth() / 2,
				this.mHmd.getScreenParams().getHeight());
		this.renderDistortionMesh(this.mLeftEyeDistortionMesh, textureId);
		GLES20.glScissor(this.mHmd.getScreenParams().getWidth() / 2, 0,
				this.mHmd.getScreenParams().getWidth() / 2, this.mHmd
	...
	public void onFovChanged(final HeadMountedDisplay hmd,
			final FieldOfView leftFov, final FieldOfView rightFov,
		...
		this.mRightEyeViewport = this.initViewportForEye(rightFov,
				this.mLeftEyeViewport.width);
		this.mMetersPerTanAngle = virtualEyeToScreenDistance;
		final ScreenParams screen = this.mHmd.getScreenParams();
		this.mXPxPerTanAngle = screen.getWidth()
				/ (screen.getWidthMeters() / this.mMetersPerTanAngle);
		this.mYPxPerTanAngle = screen.getHeight()
				/ (screen.getHeightMeters() / this.mMetersPerTanAngle);
		this.mFovsChanged = true;
		this.mViewportsChanged = true;
	}
	...
	public void updateViewports(final Viewport leftViewport,
			final Viewport rightViewport) {
		leftViewport.setViewport(
				Math.round(this.mLeftEyeViewport.x * this.mXPxPerTanAngle
						* this.mResolutionScale),
				Math.round(this.mLeftEyeViewport.y * this.mYPxPerTanAngle
						* this.mResolutionScale),
				Math.round(this.mLeftEyeViewport.width * this.mXPxPerTanAngle
						* this.mResolutionScale),
				Math.round(this.mLeftEyeViewport.height * this.mYPxPerTanAngle
						* this.mResolutionScale));
		rightViewport.setViewport(
				Math.round(this.mRightEyeViewport.x * this.mXPxPerTanAngle
						* this.mResolutionScale),
				Math.round(this.mRightEyeViewport.y * this.mYPxPerTanAngle
						* this.mResolutionScale),
				Math.round(this.mRightEyeViewport.width * this.mXPxPerTanAngle
						* this.mResolutionScale),
				Math.round(this.mRightEyeViewport.height * this.mYPxPerTanAngle
						* this.mResolutionScale));
	...
	private void updateTextureAndDistortionMesh() {
		...
		if (this.mProgramHolderAberration == null) {
			this.mProgramHolderAberration = (ProgramHolderAberration) this
					.createProgramHolder(true);
		}
		final float textureWidthTanAngle = this.mLeftEyeViewport.width
				+ this.mRightEyeViewport.width;
		final float textureHeightTanAngle = Math.max(
				this.mLeftEyeViewport.height, this.mRightEyeViewport.height);
		final int[] maxTextureSize = { 0 };
		GLES20.glGetIntegerv(GLES20.GL_MAX_TEXTURE_SIZE, maxTextureSize, 0);
		final int textureWidthPx = Math.min(
				Math.round(textureWidthTanAngle * this.mXPxPerTanAngle),
				maxTextureSize[0]);
		final int textureHeightPx = Math.min(
				Math.round(textureHeightTanAngle * this.mYPxPerTanAngle),
				maxTextureSize[0]);
		float xEyeOffsetTanAngleScreen = (screen.getWidthMeters() / 2.0f - cdp
				.getInterLensDistance() / 2.0f) / this.mMetersPerTanAngle;
		final float yEyeOffsetTanAngleScreen = (cdp
				.getVerticalDistanceToLensCenter() - screen
				.getBorderSizeMeters())
				/ this.mMetersPerTanAngle;
		this.mLeftEyeDistortionMesh = this.createDistortionMesh(
				this.mLeftEyeViewport, textureWidthTanAngle,
				textureHeightTanAngle, xEyeOffsetTanAngleScreen,
				yEyeOffsetTanAngleScreen);
		xEyeOffsetTanAngleScreen = screen.getWidthMeters()
				/ this.mMetersPerTanAngle - xEyeOffsetTanAngleScreen;
		this.mRightEyeDistortionMesh = this.createDistortionMesh(
				this.mRightEyeViewport, textureWidthTanAngle,
				textureHeightTanAngle, xEyeOffsetTanAngleScreen,
				yEyeOffsetTanAngleScreen);
	...
	private EyeViewport initViewportForEye(final FieldOfView fov,
			final float xOffset) {
	...
	private DistortionMesh createDistortionMesh(final EyeViewport eyeViewport,
			final float textureWidthTanAngle,
			final float textureHeightTanAngle,
			final float xEyeOffsetTanAngleScreen,
			final float yEyeOffsetTanAngleScreen) {
		return new DistortionMesh(this.mHmd.getCardboardDeviceParams()
				.getDistortion(), this.mHmd.getCardboardDeviceParams()
				.getDistortion(), this.mHmd.getCardboardDeviceParams()
				.getDistortion(), this.mHmd.getScreenParams().getWidthMeters()
				/ this.mMetersPerTanAngle, this.mHmd.getScreenParams()
				.getHeightMeters() / this.mMetersPerTanAngle,
	...
	private void renderDistortionMesh(final DistortionMesh mesh,
			final int textureId) {
		...
		GLES20.glVertexAttribPointer(holder.aPosition, 2, GLES20.GL_NEVER6,
				false, 36, 0 * 4);
		GLES20.glEnableVertexAttribArray(holder.aPosition);
		GLES20.glVertexAttribPointer(holder.aVignette, 1, GLES20.GL_NEVER6,
				false, 36, 2 * 4);
		GLES20.glEnableVertexAttribArray(holder.aVignette);
		GLES20.glVertexAttribPointer(holder.aBlueTextureCoord, 2,
				GLES20.GL_NEVER6, false, 36, 7 * 4);
		GLES20.glEnableVertexAttribArray(holder.aBlueTextureCoord);
		if (this.mChromaticAberrationCorrectionEnabled) {
			GLES20.glVertexAttribPointer(
					((ProgramHolderAberration) holder).aRedTextureCoord, 2,
					GLES20.GL_NEVER6, false, 36, 3 * 4);
		...
		GLES20.glBindBuffer(GLES20.GL_ELEMENT_ARRAY_BUFFER,
				mesh.mElementBufferId);
		GLES20.glDrawElements(5, mesh.nIndices, GLES20.GL_NEVER3, 0);
	}

	private float computeDistortionScale(final Distortion distortion,
			final float screenWidthM, final float interpupillaryDistanceM) {
		return distortion
	...
	private int createTexture(final int width, final int height,
			final int textureFormat, final int textureType) {
		final int[] textureIds = { 0 };
		GLES20.glGenTextures(1, textureIds, 0);
		GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, textureIds[0]);
		GLES20.glTexParameteri(GLES20.GL_TEXTURE_2D,
				GLES20.GL_STENCIL_BUFFER_BIT2, GLES20.GL_CLAMP_TO_EDGE);
		GLES20.glTexParameteri(GLES20.GL_TEXTURE_2D,
				GLES20.GL_STENCIL_BUFFER_BIT3, GLES20.GL_CLAMP_TO_EDGE);
		GLES20.glTexParameteri(GLES20.GL_TEXTURE_2D,
				GLES20.GL_STENCIL_BUFFER_BIT0, GLES20.GL_LINEAR);
		GLES20.glTexParameteri(GLES20.GL_TEXTURE_2D,
				GLES20.GL_STENCIL_BUFFER_BIT1, GLES20.GL_LINEAR);
		GLES20.glTexImage2D(GLES20.GL_TEXTURE_2D, 0, textureFormat, width,
				height, 0, textureFormat, textureType, (Buffer) null);
		return textureIds[0];
	}

	private int setupRenderTextureAndRenderbuffer(final int width,
			final int height) {
		...
		this.mTextureId = this.createTexture(width, height,
				this.mTextureFormat, this.mTextureType);
		...
		if (status != GLES20.GL_FRAMEBUFFER_COMPLETE) {
			...
			throw new RuntimeException((value.length() != 0) ? s.concat(value)
					: new String(s));
		}
	...
	private int loadShader(final int shaderType, final String source) {
		...
		if (shader != 0) {
			...
			if (compiled[0] == 0) {
				Log.e("DistortionRenderer",
						new StringBuilder(37)
								.append("Could not compile shader ")
								.append(shaderType).append(":").toString());
	...
	private int createProgram(final String vertexSource,
			final String fragmentSource) {
		final int vertexShader = this.loadShader(GLES20.GL_VERTEX_SHADER,
				vertexSource);
		if (vertexShader == 0) {
			return 0;
		}
		final int pixelShader = this.loadShader(GLES20.GL_FRAGMENT_SHADER,
				fragmentSource);
		if (pixelShader == 0) {
			return 0;
		}
	...
	private void checkGlError(final String op) {
		...
		if ((error = GLES20.glGetError()) != 0) {
			...
			Log.e(s, new StringBuilder(21 + value.length()).append(value)
					.append(": glError ").append(error).toString());
			final String value2 = String.valueOf(String.valueOf(op));
			throw new RuntimeException(new StringBuilder(21 + value2.length())
					.append(value2).append(": glError ").append(error)
					.toString());
		}
	}
	...
	private class DistortionMesh {
		...
		public DistortionMesh(final Distortion distortionRed,
				...
				final Distortion distortionBlue, final float screenWidth,
				final float screenHeight, final float xEyeOffsetScreen,
				final float yEyeOffsetScreen, final float textureWidth,
				final float textureHeight, final float xEyeOffsetTexture,
				final float yEyeOffsetTexture, final float viewportXTexture,
				final float viewportYTexture, final float viewportWidthTexture,
			...
			for (int row = 0; row < 40; ++row) {
				for (int col = 0; col < 40; ++col) {
					final float uTextureBlue = col / 39.0f
							* (viewportWidthTexture / textureWidth)
							+ viewportXTexture / textureWidth;
					final float vTextureBlue = row / 39.0f
							* (viewportHeightTexture / textureHeight)
							+ viewportYTexture / textureHeight;
					final float xTexture = uTextureBlue * textureWidth
							- xEyeOffsetTexture;
					final float yTexture = vTextureBlue * textureHeight
							- yEyeOffsetTexture;
					final float rTexture = (float) Math.sqrt(xTexture
							* xTexture + yTexture * yTexture);
					final float textureToScreenBlue = (rTexture > 0.0f) ? (distortionBlue
					...
					final float uScreen = (xScreen + xEyeOffsetScreen)
							/ screenWidth;
					final float vScreen = (yScreen + yEyeOffsetScreen)
							/ screenHeight;
					final float rScreen = rTexture * textureToScreenBlue;
					final float screenToTextureGreen = (rScreen > 0.0f) ? distortionGreen
							.distortionFactor(rScreen) : 1.0f;
					final float uTextureGreen = (xScreen * screenToTextureGreen + xEyeOffsetTexture)
							/ textureWidth;
					final float vTextureGreen = (yScreen * screenToTextureGreen + yEyeOffsetTexture)
							/ textureHeight;
					final float screenToTextureRed = (rScreen > 0.0f) ? distortionRed
							.distortionFactor(rScreen) : 1.0f;
					final float uTextureRed = (xScreen * screenToTextureRed + xEyeOffsetTexture)
							/ textureWidth;
					final float vTextureRed = (yScreen * screenToTextureRed + yEyeOffsetTexture)
							/ textureHeight;
					final float vignetteSizeTexture = 0.05f / textureToScreenBlue;
					final float dxTexture = xTexture
							+ xEyeOffsetTexture
							- clamp(xTexture + xEyeOffsetTexture,
									viewportXTexture + vignetteSizeTexture,
									viewportXTexture + viewportWidthTexture
											- vignetteSizeTexture);
					final float dyTexture = yTexture
							+ yEyeOffsetTexture
							- clamp(yTexture + yEyeOffsetTexture,
									viewportYTexture + vignetteSizeTexture,
									viewportYTexture + viewportHeightTexture
											- vignetteSizeTexture);
					final float drTexture = (float) Math.sqrt(dxTexture
							* dxTexture + dyTexture * dyTexture);
					float vignette;
					if (DistortionRenderer.this.mVignetteEnabled) {
						vignette = 1.0f - clamp(
								drTexture / vignetteSizeTexture, 0.0f, 1.0f);
					} else {
						vignette = 1.0f;
					}
			...
			GLES20.glBufferData(GLES20.GL_ARRAY_BUFFER, vertexData.length * 4,
					(Buffer) vertexBuffer, GLES20.GL_STATIC_DRAW);
			GLES20.glBindBuffer(GLES20.GL_ELEMENT_ARRAY_BUFFER,
					this.mElementBufferId);
			GLES20.glBufferData(GLES20.GL_ELEMENT_ARRAY_BUFFER,
					indexData.length * 2, (Buffer) indexBuffer,
