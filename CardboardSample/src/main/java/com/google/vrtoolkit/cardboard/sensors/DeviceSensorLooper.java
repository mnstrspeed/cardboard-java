		...
		final HandlerThread sensorThread = new HandlerThread("sensor") {
			protected void onLooperPrepared() {
				...
				for (final int sensorType : DeviceSensorLooper.INPUT_SENSORS) {
					final Sensor sensor = DeviceSensorLooper.this.mSensorManager
							.getDefaultSensor(sensorType);
					DeviceSensorLooper.this.mSensorManager.registerListener(
							DeviceSensorLooper.this.mSensorEventListener,
							sensor, 0, handler);
				}
			}
		};
