...
class UiUtils {
	...
	static void launchOrInstallCardboard(final Context context) {
		...
		final List<ResolveInfo> resolveInfos = (List<ResolveInfo>) pm
				.queryIntentActivities(settingsIntent, 0);
			...
			public void onClick(final DialogInterface dialog, final int id) {
				try {
					...
					Toast.makeText(context.getApplicationContext(),
							(CharSequence) "No browser to open website.", 1)
							.show();
				}
			}
		};
		final FragmentManager fragmentManager = ((Activity) context)
				.getFragmentManager();
		final DialogFragment dialog = new SettingsDialogFragment(
				(DialogStrings) new InstallDialogStrings(), listener);
		dialog.show(fragmentManager, "InstallCardboardDialog");
	}

	private static void showConfigureDialog(final Context context,
			final Intent intent) {
		final DialogInterface.OnClickListener listener = (DialogInterface.OnClickListener) new DialogInterface.OnClickListener() {
			public void onClick(final DialogInterface dialog, final int id) {
				try {
		...
		final FragmentManager fragmentManager = ((Activity) context)
				.getFragmentManager();
		final DialogFragment dialog = new SettingsDialogFragment(
				(DialogStrings) new ConfigureDialogStrings(), listener);
	...
	private static class SettingsDialogFragment extends DialogFragment {
		...
		private SettingsDialogFragment(final DialogStrings dialogStrings,
		...
		public Dialog onCreateDialog(final Bundle savedInstanceState) {
			...
			builder.setTitle((CharSequence) this.mDialogStrings.mTitle)
					.setMessage((CharSequence) this.mDialogStrings.mMessage)
					.setPositiveButton(
							(CharSequence) this.mDialogStrings.mPositiveButtonText,
							this.mPositiveButtonListener)
					.setNegativeButton(
							(CharSequence) this.mDialogStrings.mNegativeButtonText,
