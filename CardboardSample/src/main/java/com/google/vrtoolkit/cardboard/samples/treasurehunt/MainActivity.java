...
public class MainActivity extends CardboardActivity implements
	...
	public void onSurfaceCreated(EGLConfig config) {
		...
		ByteBuffer bbVertices = ByteBuffer
				.allocateDirect(DATA.CUBE_COORDS.length * 4);
		bbVertices.order(ByteOrder.nativeOrder());
		mCubeVertices = bbVertices.asFloatBuffer();
		mCubeVertices.put(DATA.CUBE_COORDS);
		mCubeVertices.position(0);

		ByteBuffer bbColors = ByteBuffer
				.allocateDirect(DATA.CUBE_COLORS.length * 4);
		...
		ByteBuffer bbFoundColors = ByteBuffer
				.allocateDirect(DATA.CUBE_FOUND_COLORS.length * 4);
		...
		ByteBuffer bbNormals = ByteBuffer
				.allocateDirect(DATA.CUBE_NORMALS.length * 4);
		...
		// make a floor
		ByteBuffer bbFloorVertices = ByteBuffer
				.allocateDirect(DATA.FLOOR_COORDS.length * 4);
		bbFloorVertices.order(ByteOrder.nativeOrder());
		mFloorVertices = bbFloorVertices.asFloatBuffer();
		mFloorVertices.put(DATA.FLOOR_COORDS);
		mFloorVertices.position(0);

		ByteBuffer bbFloorNormals = ByteBuffer
				.allocateDirect(DATA.FLOOR_NORMALS.length * 4);
		bbFloorNormals.order(ByteOrder.nativeOrder());
		mFloorNormals = bbFloorNormals.asFloatBuffer();
		mFloorNormals.put(DATA.FLOOR_NORMALS);
		mFloorNormals.position(0);

		ByteBuffer bbFloorColors = ByteBuffer
				.allocateDirect(DATA.FLOOR_COLORS.length * 4);
		...
		mCubePositionParam = GLES20.glGetAttribLocation(mCubeProgram,
				"a_Position");
		...
		mCubeModelViewParam = GLES20.glGetUniformLocation(mCubeProgram,
				"u_MVMatrix");
		mCubeModelViewProjectionParam = GLES20.glGetUniformLocation(
				mCubeProgram, "u_MVP");
		mCubeLightPosParam = GLES20.glGetUniformLocation(mCubeProgram,
				"u_LightPos");
		...
		mFloorModelParam = GLES20
				.glGetUniformLocation(mFloorProgram, "u_Model");
		mFloorModelViewParam = GLES20.glGetUniformLocation(mFloorProgram,
				"u_MVMatrix");
		mFloorModelViewProjectionParam = GLES20.glGetUniformLocation(
				mFloorProgram, "u_MVP");
		mFloorLightPosParam = GLES20.glGetUniformLocation(mFloorProgram,
				"u_LightPos");

		mFloorPositionParam = GLES20.glGetAttribLocation(mFloorProgram,
				"a_Position");
		mFloorNormalParam = GLES20.glGetAttribLocation(mFloorProgram,
				"a_Normal");
	...
	public void drawCube() {
		...
		GLES20.glVertexAttribPointer(mCubePositionParam, COORDS_PER_VERTEX,
				GLES20.GL_FLOAT, false, 0, mCubeVertices);

		// Set the ModelViewProjection matrix in the shader.
		GLES20.glUniformMatrix4fv(mCubeModelViewProjectionParam, 1, false,
				mModelViewProjection, 0);

		// Set the normal positions of the cube, again for shading
		GLES20.glVertexAttribPointer(mCubeNormalParam, 3, GLES20.GL_FLOAT,
				false, 0, mCubeNormals);
		GLES20.glVertexAttribPointer(mCubeColorParam, 4, GLES20.GL_FLOAT,
				false, 0, isLookingAtObject() ? mCubeFoundColors : mCubeColors);
	...
	public void drawFloor() {
		...
		GLES20.glUniformMatrix4fv(mFloorModelViewProjectionParam, 1, false,
				mModelViewProjection, 0);
		GLES20.glVertexAttribPointer(mFloorPositionParam, COORDS_PER_VERTEX,
				GLES20.GL_FLOAT, false, 0, mFloorVertices);
		GLES20.glVertexAttribPointer(mFloorNormalParam, 3, GLES20.GL_FLOAT,
				false, 0, mFloorNormals);
		GLES20.glVertexAttribPointer(mFloorColorParam, 4, GLES20.GL_FLOAT,
				false, 0, mFloorColors);
