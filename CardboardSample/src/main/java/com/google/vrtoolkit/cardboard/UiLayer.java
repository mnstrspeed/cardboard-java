...
class UiLayer {
	...
	UiLayer(final Context context) {
		...
		this.mSettingsButtonRenderer = new SettingsButtonRenderer(this.mShader,
				buttonWidthPx);
		this.mAlignmentMarkerRenderer = new AlignmentMarkerRenderer(
				this.mShader, this.mTouchWidthPx, 4.0f * density);
	...
	void updateViewport(final Viewport viewport) {
		synchronized (this) {
			...
			this.mTouchRect = new Rect((w - this.mTouchWidthPx) / 2, h
					- this.mTouchWidthPx, (w + this.mTouchWidthPx) / 2, h);
			this.mViewport.setViewport(viewport.x, viewport.y, viewport.width,
					viewport.height);
			this.mShouldUpdateViewport = true;
		}
	}
	...
	void draw() {
		if (!this.getSettingsButtonEnabled()
				&& !this.getAlignmentMarkerEnabled()) {
			return;
		}
		if (!this.initialized) {
			this.initializeGl();
	...
	boolean onTouchEvent(final MotionEvent e) {
		...
		synchronized (this) {
			...
			touchWithinBounds = this.mTouchRect.contains((int) e.getX(),
					(int) e.getY());
		}
	...
	private void setPressed(final boolean pressed) {
		if (this.mSettingsButtonRenderer != null) {
			this.mSettingsButtonRenderer.setColor(pressed ? -12303292
					: -3355444);
		}
	}
	...
	private static void checkGlError(final String op) {
		...
		if ((error = GLES20.glGetError()) != 0) {
			...
			Log.e(tag, new StringBuilder(21 + value.length()).append(value)
	...
	private static class ShaderProgram {
		...
		void initializeGl() {
			...
			this.aPosition = GLES20.glGetAttribLocation(this.program,
					"aPosition");
			...
			this.uMvpMatrix = GLES20.glGetUniformLocation(this.program,
					"uMVPMatrix");
			if (this.uMvpMatrix == -1) {
				throw new RuntimeException(
						"Could not get uniform location for uMVPMatrix");
			}
			this.uColor = GLES20.glGetUniformLocation(this.program, "uColor");
			if (this.uColor == -1) {
				throw new RuntimeException(
						"Could not get uniform location for uColor");
			}
		}
	...
	private static class MeshRenderer {
		...
		void genAndBindBuffers(final float[] vertexData, final short[] indexData) {
			...
			final ShortBuffer indexBuffer = ByteBuffer
					.allocateDirect(this.mNumIndices * 4)
			...
			GLES20.glBufferData(GLES20.GL_ELEMENT_ARRAY_BUFFER,
					indexData.length * 4, (Buffer) indexBuffer,
		...
		void draw() {
			...
			GLES20.glUniformMatrix4fv(this.mShader.uMvpMatrix, 1, false,
					this.mMvp, 0);
			GLES20.glBindBuffer(GLES20.GL_ARRAY_BUFFER, this.mArrayBufferId);
			GLES20.glVertexAttribPointer(this.mShader.aPosition, 2,
					GLES20.GL_NEVER6, false, 8, 0);
	...
	private static class AlignmentMarkerRenderer extends MeshRenderer {
		...
		AlignmentMarkerRenderer(final ShaderProgram shader,
				final float verticalBorderPaddingPx, final float lineThicknessPx) {
			super(shader);
		...
		void initializeGl() {
			final float[] vertexData = { 1.0f, 1.0f, -1.0f, 1.0f, 1.0f, -1.0f,
					-1.0f, -1.0f };
		...
		void updateViewport(final Viewport viewport) {
			...
			final float yScale = 1.0f - 2.0f * this.mVerticalBorderPaddingPx
					/ viewport.height;
		...
		void draw() {
			GLES20.glUseProgram(this.mShader.program);
			GLES20.glUniform4f(this.mShader.uColor,
					Color.red(AlignmentMarkerRenderer.COLOR) / 255.0f,
					Color.green(AlignmentMarkerRenderer.COLOR) / 255.0f,
					Color.blue(AlignmentMarkerRenderer.COLOR) / 255.0f,
					Color.alpha(AlignmentMarkerRenderer.COLOR) / 255.0f);
			super.draw();
		}
	...
	private static class SettingsButtonRenderer extends MeshRenderer {
		...
		SettingsButtonRenderer(final ShaderProgram shader,
				final int buttonWidthPx) {
			super(shader);
			this.mColor = -3355444;
		...
		void initializeGl() {
			...
			for (int i = 0; i < numVerticesPerRim; ++i) {
				...
				vertexData[2 * i] = r
						* (float) Math.cos(Math.toRadians(90.0f - theta));
				vertexData[2 * i + 1] = r
						* (float) Math.sin(Math.toRadians(90.0f - theta));
			}
			...
			for (int j = 0; j < numVerticesPerRim; ++j) {
				...
				vertexData[innerStartingIndex + 2 * j] = 0.3125f * (float) Math
						.cos(Math.toRadians(90.0f - theta2));
				vertexData[innerStartingIndex + 2 * j + 1] = 0.3125f * (float) Math
						.sin(Math.toRadians(90.0f - theta2));
			}
		...
		void draw() {
			...
			synchronized (this) {
				GLES20.glUniform4f(this.mShader.uColor,
						Color.red(this.mColor) / 255.0f,
						Color.green(this.mColor) / 255.0f,
						Color.blue(this.mColor) / 255.0f,
						Color.alpha(this.mColor) / 255.0f);
			}
			super.draw();
		}
	}
