...
public class PhoneParams {
	...
	static Phone.PhoneParams readFromInputStream(final InputStream inputStream) {
		...
		try {
			...
			return (Phone.PhoneParams) MessageNano.mergeFrom(
					(MessageNano) new Phone.PhoneParams(), protoBytes);
			...
			Log.w(tag2, (value2.length() != 0) ? s2.concat(value2)
					: new String(s2));
		}
		return null;
	}
	...
	static Phone.PhoneParams readFromExternalStorage() {
		try {
			...
			Log.d(tag,
					new StringBuilder(43 + value.length())
							.append("Cardboard phone parameters file not found: ")
