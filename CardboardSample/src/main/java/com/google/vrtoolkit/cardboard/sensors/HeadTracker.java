...
public class HeadTracker implements SensorEventListener {
	...
	public HeadTracker(final SensorEventProvider sensorEventProvider,
			final Clock clock, final Display display) {
		super();
	...
	public void onSensorChanged(final SensorEvent event) {
		if (event.sensor.getType() == 1) {
			this.mLatestAcc.set(event.values[0], event.values[1],
					event.values[2]);
			...
			this.mLatestGyro.set(event.values[0], event.values[1],
					event.values[2]);
	...
	public void stopTracking() {
		...
		this.mSensorEventProvider
				.unregisterListener((SensorEventListener) this);
	...
	public void setGyroBias(final float[] gyroBias) {
		...
		if (gyroBias.length != 3) {
			throw new IllegalArgumentException(
					"Gyro bias should be an array of 3 values");
		}
