...
public class MagnetSensor {
	...
	public void setOnCardboardTriggerListener(
			final OnCardboardTriggerListener listener) {
					...
					this.mHandler.post((Runnable) new Runnable() {
						...
						public void run() {
							if (TriggerDetector.this.mListener != null) {
								TriggerDetector.this.mListener
										.onCardboardTrigger();
							}
						}
					});
	...
	private static class ThresholdTriggerDetector extends TriggerDetector {
		...
		public ThresholdTriggerDetector(final Context context, final int t1,
				final int t2) {
			super(context);
			this.mLastFiring = 0L;
			this.mSensorData = new ArrayList<float[]>();
		...
		private void evaluateModel(final long time) {
			if (time - this.mLastFiring < 350000000L
					|| this.mSensorData.size() < 2) {
				return;
			}
			final float[] baseline = this.mSensorData.get(this.mSensorData
					.size() - 1);
			...
			final float min1 = this.computeMinimum(Arrays.copyOfRange(offsets,
					0, startSecondSegment));
			final float max2 = this.computeMaximum(Arrays.copyOfRange(offsets,
					startSecondSegment, this.mSensorData.size()));
			if (min1 < ThresholdTriggerDetector.mT1
					&& max2 > ThresholdTriggerDetector.mT2) {
				this.mLastFiring = time;
				this.handleButtonPressed();
			}
		}

		private void computeOffsets(final float[] offsets,
				final float[] baseline) {
			for (int i = 0; i < this.mSensorData.size(); ++i) {
				final float[] point = this.mSensorData.get(i);
				final float[] o = { point[0] - baseline[0],
						point[1] - baseline[1], point[2] - baseline[2] };
				final float magnitude = (float) Math.sqrt(o[0] * o[0] + o[1]
						* o[1] + o[2] * o[2]);
				offsets[i] = magnitude;
			}
		}
	...
	private static class VectorTriggerDetector extends TriggerDetector {
		...
		public VectorTriggerDetector(final Context context,
				final int xThreshold, final int yThreshold, final int zThreshold) {
			super(context);
			this.mLastFiring = 0L;
			this.mSensorData = new ArrayList<float[]>();
			this.mSensorTimes = new ArrayList<Long>();
		...
		private void evaluateModel(final long time) {
			...
			final float[] currentValues = this.mSensorData.get(this.mSensorData
					.size() - 1);
