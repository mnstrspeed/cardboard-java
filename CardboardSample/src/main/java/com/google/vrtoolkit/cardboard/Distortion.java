...
public class Distortion {
	...
	public void setCoefficients(final float[] coefficients) {
		this.mCoefficients = ((coefficients != null) ? coefficients.clone()
				: new float[0]);
	}
	...
	private static double[] solveLeastSquares(final double[][] matA,
			final double[] vecY) {
		...
		if (numCoefficients != 2) {
			throw new RuntimeException(
					new StringBuilder(78)
							...
							.append(numCoefficients).append(" given.")
							.toString());
		}
		final double det = matATA[0][0] * matATA[1][1] - matATA[0][1]
				* matATA[1][0];
