...
public class CardboardView extends GLSurfaceView {
	...
	public void setRenderer(final StereoRenderer renderer) {
		this.setRenderer((renderer != null) ? new StereoRendererHelper(renderer)
				: ((Renderer) null));
	}

	public void getCurrentEyeParams(final HeadTransform head,
			final Eye leftEye, final Eye rightEye, final Eye monocular) {
		this.mRendererHelper.getCurrentEyeParams(head, leftEye, rightEye,
				monocular);
	}
	...
	public void updateCardboardDeviceParams(
		...
		if (this.mHmdManager.updateCardboardDeviceParams(cardboardDeviceParams)) {
			this.mRendererHelper.setCardboardDeviceParams(this
	...
	public void setDistortionCorrectionTextureFormat(final int textureFormat,
			final int textureType) {
		this.mRendererHelper.setDistortionCorrectionTextureFormat(
	...
	public void onResume() {
		...
		final Phone.PhoneParams phoneParams = PhoneParams
	...
	public void onDetachedFromWindow() {
		if (this.mRendererSet && this.mShutdownLatch == null) {
			...
			try {
				...
				Log.e(s, (value.length() != 0) ? s2.concat(value) : new String(
						s2));
			}
	...
	private class RendererHelper implements GLSurfaceView.Renderer {
		...
		public RendererHelper() {
			super();
			this.mHmd = new HeadMountedDisplay(
					CardboardView.this.getHeadMountedDisplay());
			this.mHeadTransform = new HeadTransform();
			this.mMonocular = new Eye(0);
			this.mLeftEye = new Eye(1);
			this.mRightEye = new Eye(2);
			this.updateFieldOfView(this.mLeftEye.getFov(),
					this.mRightEye.getFov());
			(this.mDistortionRenderer = new DistortionRenderer())
					.setRestoreGLStateEnabled(CardboardView.this.mRestoreGLStateEnabled);
			this.mDistortionRenderer
					.setChromaticAberrationCorrectionEnabled(CardboardView.this.mChromaticAberrationCorrectionEnabled);
			this.mDistortionRenderer
					.setVignetteEnabled(CardboardView.this.mVignetteEnabled);
		...
		public void shutdown() {
			CardboardView.this.queueEvent(new Runnable() {
				@Override
				public void run() {
					if (RendererHelper.this.mRenderer != null
							&& RendererHelper.this.mSurfaceCreated) {
		...
		public void setCardboardDeviceParams(
				final CardboardDeviceParams newParams) {
			final CardboardDeviceParams deviceParams = new CardboardDeviceParams(
					newParams);
			CardboardView.this.queueEvent(new Runnable() {
				@Override
				public void run() {
					RendererHelper.this.mHmd
							.setCardboardDeviceParams(deviceParams);
		...
		public void setDistortionCorrectionScale(final float scale) {
			CardboardView.this.queueEvent(new Runnable() {
				@Override
				public void run() {
					RendererHelper.this.mDistortionRenderer
		...
		public void setVRModeEnabled(final boolean enabled) {
			CardboardView.this.queueEvent(new Runnable() {
				...
				public void run() {
					...
					RendererHelper.this.mProjectionChanged = true;
					RendererHelper.this.onSurfaceChanged(null,
							RendererHelper.this.mHmd.getScreenParams()
									.getWidth(), RendererHelper.this.mHmd
		...
		public void setRestoreGLStateEnabled(final boolean enabled) {
			CardboardView.this.queueEvent(new Runnable() {
				...
				public void run() {
					RendererHelper.this.mDistortionRenderer
							.setRestoreGLStateEnabled(enabled);
				}
			});
		}

		public void setChromaticAberrationCorrectionEnabled(
				final boolean enabled) {
			CardboardView.this.queueEvent(new Runnable() {
				@Override
				public void run() {
					RendererHelper.this.mDistortionRenderer
							.setChromaticAberrationCorrectionEnabled(enabled);
				}
			});
		}
		...
		public void setDistortionCorrectionTextureFormat(
				final int textureFormat, final int textureType) {
			CardboardView.this.queueEvent(new Runnable() {
				@Override
				public void run() {
					RendererHelper.this.mDistortionRenderer.setTextureFormat(
							textureFormat, textureType);
				}
			});
		}
		...
		public void getCurrentEyeParams(final HeadTransform head,
			...
			CardboardView.this.queueEvent(new Runnable() {
				@Override
				public void run() {
					RendererHelper.this.getFrameParams(head, leftEye, rightEye,
							monocular);
					finished.countDown();
				}
			});
		...
		private void getFrameParams(final HeadTransform head,
				final Eye leftEye, final Eye rightEye, final Eye monocular) {
			final CardboardDeviceParams cdp = this.mHmd
					.getCardboardDeviceParams();
			final ScreenParams screen = this.mHmd.getScreenParams();
			CardboardView.this.mHeadTracker.getLastHeadView(head.getHeadView(),
					0);
			...
			if (this.mVRMode) {
				...
				Matrix.translateM(this.mRightEyeTranslate, 0,
						-halfInterpupillaryDistance, 0.0f, 0.0f);
				Matrix.multiplyMM(leftEye.getEyeView(), 0,
						this.mLeftEyeTranslate, 0, head.getHeadView(), 0);
				Matrix.multiplyMM(rightEye.getEyeView(), 0,
						this.mRightEyeTranslate, 0, head.getHeadView(), 0);
			} else {
				System.arraycopy(head.getHeadView(), 0, monocular.getEyeView(),
						0, head.getHeadView().length);
			}
			if (this.mProjectionChanged) {
				monocular.getViewport().setViewport(0, 0, screen.getWidth(),
						screen.getHeight());
				CardboardView.this.mUiLayer.updateViewport(monocular
						.getViewport());
				if (!this.mVRMode) {
					...
					this.mDistortionRenderer.onFovChanged(this.mHmd,
							leftEye.getFov(), rightEye.getFov(),
							this.getVirtualEyeToScreenDistance());
				} else {
			...
			if (this.mDistortionCorrectionEnabled
					&& this.mDistortionRenderer.haveViewportsChanged()) {
				this.mDistortionRenderer.updateViewports(leftEye.getViewport(),
		...
		public void onDrawFrame(final GL10 gl) {
			if (this.mRenderer == null || !this.mSurfaceCreated
					|| this.mInvalidSurfaceSize) {
				return;
			}
			this.getFrameParams(this.mHeadTransform, this.mLeftEye,
					this.mRightEye, this.mMonocular);
			if (this.mVRMode) {
				if (this.mDistortionCorrectionEnabled) {
		...
		public void onSurfaceChanged(final GL10 gl, final int width,
			...
			if (width != screen.getWidth() || height != screen.getHeight()) {
				if (!this.mInvalidSurfaceSize) {
					...
					Log.w("CardboardView",
							new StringBuilder(124)
									.append("Surface size ")
									.append(width)
									.append("x")
									.append(height)
									.append(" does not match the expected screen size ")
									.append(screen.getWidth()).append("x")
									.append(screen.getHeight())
									.append(". Rendering is disabled.")
									.toString());
				}
		...
		private void updateFieldOfView(final FieldOfView leftEyeFov,
				final FieldOfView rightEyeFov) {
			final CardboardDeviceParams cdp = this.mHmd
					.getCardboardDeviceParams();
			final ScreenParams screen = this.mHmd.getScreenParams();
			final Distortion distortion = cdp.getDistortion();
			final float eyeToScreenDist = this.getVirtualEyeToScreenDistance();
			final float outerDist = (screen.getWidthMeters() - cdp
					.getInterLensDistance()) / 2.0f;
			final float innerDist = cdp.getInterLensDistance() / 2.0f;
			final float bottomDist = cdp.getVerticalDistanceToLensCenter()
					- screen.getBorderSizeMeters();
			final float topDist = screen.getHeightMeters()
					+ screen.getBorderSizeMeters()
					- cdp.getVerticalDistanceToLensCenter();
			final float outerAngle = (float) Math.toDegrees(Math
					.atan(distortion.distort(outerDist / eyeToScreenDist)));
			final float innerAngle = (float) Math.toDegrees(Math
					.atan(distortion.distort(innerDist / eyeToScreenDist)));
			final float bottomAngle = (float) Math.toDegrees(Math
					.atan(distortion.distort(bottomDist / eyeToScreenDist)));
			final float topAngle = (float) Math.toDegrees(Math.atan(distortion
					.distort(topDist / eyeToScreenDist)));
			leftEyeFov.setLeft(Math.min(outerAngle, cdp.getLeftEyeMaxFov()
					.getLeft()));
			leftEyeFov.setRight(Math.min(innerAngle, cdp.getLeftEyeMaxFov()
					.getRight()));
			leftEyeFov.setBottom(Math.min(bottomAngle, cdp.getLeftEyeMaxFov()
					.getBottom()));
			leftEyeFov.setTop(Math.min(topAngle, cdp.getLeftEyeMaxFov()
					.getTop()));
		...
		private void updateMonocularFieldOfView(final FieldOfView monocularFov) {
			...
			final float monocularLeftFov = (float) Math.toDegrees(Math
					.atan(Math.tan(Math.toRadians(monocularBottomFov))
							* screen.getWidthMeters()
		...
		private void updateUndistortedFovAndViewport() {
			...
			final float left = screen.getWidthMeters() / 2.0f
					- halfLensDistance;
			final float right = halfLensDistance;
			final float bottom = cdp.getVerticalDistanceToLensCenter()
					- screen.getBorderSizeMeters();
			final float top = screen.getBorderSizeMeters()
					+ screen.getHeightMeters()
					- cdp.getVerticalDistanceToLensCenter();
			final FieldOfView leftEyeFov = this.mLeftEye.getFov();
			leftEyeFov.setLeft((float) Math.toDegrees(Math.atan2(left,
					eyeToScreen)));
			leftEyeFov.setRight((float) Math.toDegrees(Math.atan2(right,
					eyeToScreen)));
			leftEyeFov.setBottom((float) Math.toDegrees(Math.atan2(bottom,
					eyeToScreen)));
			...
			this.mRightEye.getViewport().setViewport(screen.getWidth() / 2, 0,
					screen.getWidth() / 2, screen.getHeight());
		}

		private float getVirtualEyeToScreenDistance() {
			return this.mHmd.getCardboardDeviceParams()
	...
	private class StereoRendererHelper implements Renderer {
		...
		@Override
		public void onDrawFrame(final HeadTransform head, final Eye leftEye,
				final Eye rightEye) {
