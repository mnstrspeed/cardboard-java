...
public class CardboardDeviceParams {
	...
	public CardboardDeviceParams(final CardboardDevice.DeviceParams params) {
		...
		this.mLeftEyeMaxFov = FieldOfView
	...
	public static boolean isOriginalCardboardDeviceUri(final Uri uri) {
		return CardboardDeviceParams.URI_ORIGINAL_CARDBOARD_QR_CODE
				.equals((Object) uri)
				|| (CardboardDeviceParams.URI_ORIGINAL_CARDBOARD_NFC
						.getScheme().equals(uri.getScheme()) && CardboardDeviceParams.URI_ORIGINAL_CARDBOARD_NFC
	...
	public static CardboardDeviceParams createFromUri(final Uri uri) {
		...
		if (!isCardboardDeviceUri(uri)) {
			Log.w("CardboardDeviceParams", String.format(
					"URI \"%s\" not recognized as cardboard device.", uri));
			return null;
		}
		...
		if (paramsEncoded != null) {
			try {
				...
				params = (CardboardDevice.DeviceParams) MessageNano
						.mergeFrom(
								(MessageNano) new CardboardDevice.DeviceParams(),
								bytes);
				Log.d("CardboardDeviceParams",
						"Read cardboard params from URI.");
			} catch (Exception e) {
				...
				Log.w(s, (value.length() != 0) ? s2.concat(value) : new String(
						s2));
			}
		} else {
	...
	public static CardboardDeviceParams createFromInputStream(
		...
		try {
			...
			return new CardboardDeviceParams(
					(CardboardDevice.DeviceParams) MessageNano.mergeFrom(
							(MessageNano) new CardboardDevice.DeviceParams(),
							protoBytes));
			...
			Log.w(s3, (value2.length() != 0) ? s4.concat(value2) : new String(
					s4));
		}
		return null;
	}
	...
	public void setVerticalDistanceToLensCenter(
			final float verticalDistanceToLensCenter) {
	...
	public boolean equals(final Object other) {
		...
		return this.mVendor.equals(o.mVendor)
				&& this.mModel.equals(o.mModel)
				&& this.mInterLensDistance == o.mInterLensDistance
				&& this.mVerticalDistanceToLensCenter == o.mVerticalDistanceToLensCenter
				&& this.mScreenToLensDistance == o.mScreenToLensDistance
				&& this.mLeftEyeMaxFov.equals(o.mLeftEyeMaxFov)
				&& this.mDistortion.equals(o.mDistortion)
				&& this.mHasMagnet == o.mHasMagnet;
	}
	...
	public String toString() {
		...
		final StringBuilder append2 = append.append(new StringBuilder(
				12 + value.length()).append("  vendor: ").append(value)
				.append(",\n").toString());
		final String value2 = String.valueOf(String.valueOf(this.mModel));
		final StringBuilder append3 = append2
				.append(new StringBuilder(11 + value2.length())
						.append("  model: ").append(value2).append(",\n")
						.toString())
				.append(new StringBuilder(40).append("  inter_lens_distance: ")
						.append(this.mInterLensDistance).append(",\n")
						.toString())
				.append(new StringBuilder(53)
						.append("  vertical_distance_to_lens_center: ")
						.append(this.mVerticalDistanceToLensCenter)
						.append(",\n").toString())
				.append(new StringBuilder(44)
						.append("  screen_to_lens_distance: ")
						.append(this.mScreenToLensDistance).append(",\n")
						.toString());
		final String value3 = String.valueOf(String.valueOf(this.mLeftEyeMaxFov
				.toString().replace("\n", "\n  ")));
		final StringBuilder append4 = append3.append(new StringBuilder(
				22 + value3.length()).append("  left_eye_max_fov: ")
				.append(value3).append(",\n").toString());
		final String value4 = String.valueOf(String.valueOf(this.mDistortion
				.toString().replace("\n", "\n  ")));
		return append4
				.append(new StringBuilder(16 + value4.length())
						.append("  distortion: ").append(value4).append(",\n")
						.toString())
				.append(new StringBuilder(17).append("  magnet: ")
						.append(this.mHasMagnet).append(",\n").toString())
				.append("}\n").toString();
	}
	...
	static {
		URI_ORIGINAL_CARDBOARD_NFC = new Uri.Builder().scheme("cardboard")
				.authority("v1.0.0").build();
		URI_ORIGINAL_CARDBOARD_QR_CODE = new Uri.Builder().scheme("http")
				.authority("g.co").appendEncodedPath("cardboard").build();
	}
}
